...
# DATA SIMULATION
## Demographics 
vaccine_ad_campaigns <- data.frame(id = 1:5000,  
  age = sample(18:65, 5000, replace = TRUE), 
...
vaccine_ad_campaigns$b_had_vaccine <- mapply(function(education, income) {
  ...
  if (income %in% c("$50,000–$74,999", "$75,000–$99,999", "$100,000 or more")) {
    prob_vaccine <- prob_vaccine + 0.1}
  return(sample(c("Yes", "No"), 1, replace = TRUE, prob = c(prob_vaccine, 1 - prob_vaccine)))}, vaccine_ad_campaigns$education, vaccine_ad_campaigns$income)

### WHY OR WHY NOT?
vaccine_ad_campaigns$b_reasons_vaccine <- mapply(function(b_had_vaccine) {
  ...
  return(paste(reasons, collapse = ", "))}, vaccine_ad_campaigns$b_had_vaccine)

### ARE YOU CONSIDERING GETTING THE VACCINE?
vaccine_ad_campaigns$b_considering_vaccine <- ifelse(vaccine_ad_campaigns$b_had_vaccine == "No", sample(c("Yes", "No"), 5000, replace = TRUE, prob = c(0.5, 0.5)), NA) 

### HAVE YOU EVER TESTED POSITIVE FOR COVID?
vaccine_ad_campaigns$b_tested_positive <- mapply(function(vaccinated_status) {
  if (vaccinated_status == "Yes") {# Lower chance of testing positive for vaccinated (10% chance)
    ...
    return(sample(c("Yes", "No"), 1, prob = c(0.25, 0.75)))}}, vaccine_ad_campaigns$b_had_vaccine)
...
vaccine_ad_campaigns$b_worried_about_covid <- ifelse(
  vaccine_ad_campaigns$b_tested_positive == "No", 
...
vaccine_ad_campaigns$is_immunocompromised <- sample(c("Yes", "No"), 5000, replace = TRUE, prob = c(0.1, 0.9))  

### ARE THERE MEMBERS IN YOUR HOUSEHOLD (EXCLUDING YOURSELF) WHO ARE IMMUNOCOMPROMISED?
vaccine_ad_campaigns$household_immunocompromised <- sample(c("Yes", "No"), 5000, replace = TRUE, prob = c(0.2, 0.8))

### HAVE YOU EVER BEEN VACCINATED FOR OTHER DISEASES? (Provide list of possible other vaccinations)
vaccine_ad_campaigns$other_vaccinations <- sample(c("Yes, for all or almost all", "Yes, but only a few diseases", "No", "Unsure"), 5000, replace = TRUE)

### TO WHAT EXTENT ARE YOU WORRIED ABOUT THE LONG-TERM EFFECTS OF COVID-19? (i.e. What's known as "Long Covid")
vaccine_ad_campaigns$b_long_term_concerns <- mapply(function(b_had_vaccine) {
...
}, vaccine_ad_campaigns$b_had_vaccine)

### TO WHAT EXTENT ARE YOU WORRIED ABOUT THE LONG-TERM EFFECTS OF THE COVID-19 VACCINE?
vaccine_ad_campaigns$b_vaccine_long_term_concerns <- mapply(function(b_had_vaccine) {
...
vaccine_ad_campaigns$b_vaccine_mandates <- mapply(function(b_had_vaccine) {
  if (b_had_vaccine == "Yes") {
    ...
    return(sample(c("Yes, for everyone", "Yes, but only for certain groups", "No, it should not be mandatory", "Unsure"), 1, replace = TRUE, prob = c(0.05, 0.4, 0.45, 0.1)))}}, vaccine_ad_campaigns$b_had_vaccine)
...
vaccine_ad_campaigns$assignment <- sample(c("Reason", "Emotions", "None"), 5000, replace = TRUE, prob = c(1/3, 1/3, 1/3))
```

```{r}
# DATA SIMULATION: Endline Questions
# Select a sample of 4500 respondents for the endline survey
endline_sample <- sample(nrow(vaccine_ad_campaigns), 4500)
# Create the endline_survey data frame with only the sampled 4500 respondents
endline_survey <- vaccine_ad_campaigns[endline_sample, ]

### HAVE YOU HAD THE VACCINE YET?
# Initialize the 'e_had_vaccine' column from 'b_had_vaccine'
endline_survey$e_had_vaccine <- endline_survey$b_had_vaccine

# Apply changes to 'e_had_vaccine' based on the conditions for the sampled respondents
endline_survey$e_had_vaccine <- mapply(function(education, income, assignment, b_had_vaccine) {# Only modify "No" responses
  ...
  return("Yes")}, endline_survey$education, endline_survey$income, endline_survey$assignment, endline_survey$b_had_vaccine)

### WHY OR WHY NOT?
# Initialize reasons for 'e_reasons_vaccine' by copying the baseline reasons
endline_survey$e_reasons_vaccine <- endline_survey$b_reasons_vaccine

# Now update reasons for those who changed their answer from "No" to "Yes"
for (i in 1:nrow(endline_survey)) {
  if (endline_survey$b_had_vaccine[i] == "No" && endline_survey$e_had_vaccine[i] == "Yes") {# Update reasons for those who decided to get the vaccine after initially refusing
    ...
    endline_survey$e_reasons_vaccine[i] <- paste(reasons, collapse = ", ")} else {# For everyone else (those who didn't change their answer), keep the baseline reasons
    endline_survey$e_reasons_vaccine[i] <- endline_survey$b_reasons_vaccine[i]}}

### HAVE YOU EVER TESTED POSITIVE FOR COVID?
# Initialize the 'e_tested_positive' column from 'b_tested_positive'
endline_survey$e_tested_positive <- endline_survey$b_tested_positive

# Apply changes to 'e_tested_positive' based on the conditions
endline_survey$e_tested_positive <- mapply(function(education, income, assignment, b_tested_positive, b_had_vaccine) {# Only modify "No" responses
  ...
  return("Yes")}, endline_survey$education, endline_survey$income, endline_survey$assignment, endline_survey$b_tested_positive, endline_survey$b_had_vaccine)

### ARE YOU WORRIED ABOUT COVID?
# Copy baseline answers into e_worried_about_covid for endline_survey
endline_survey$e_worried_about_covid <- endline_survey$b_worried_about_covid

# Change answers for those who did not test positive and answered b_had_vaccine == "No"
for (i in 1:nrow(endline_survey)) {if (endline_survey$b_tested_positive[i] == "No" && endline_survey$b_had_vaccine[i] == "No") {# Update e_worried_about_covid for these individuals based on the sample distribution
    endline_survey$e_worried_about_covid[i] <- sample(1:5, 1, replace = TRUE, prob = c(0.1, 0.2, 0.4, 0.2, 0.1))}}
...
for (i in endline_sample) {# Check if 'assignment' is not NA and proceed with the assignment
  if (!is.na(endline_survey$assignment[i])) {
    if (endline_survey$assignment[i] == "Reason") {
# Make those exposed to the "Reason" campaign less worried about COVID-19 (scale 1-5)
      endline_survey$e_long_term_concerns[i] <- sample(1:5, 1, replace = TRUE, prob = c(0.1, 0.3, 0.5, 0.1, 0)) 
# Make those exposed to the "Reason" campaign less worried about the long-term effects of the COVID-19 vaccine (scale 1-5)
      endline_survey$e_vaccine_long_term_concerns[i] <- sample(1:5, 1, replace = TRUE, prob = c(0.1, 0.3, 0.5, 0.1, 0))} else if (endline_survey$assignment[i] == "Emotions") {# Make those exposed to the "Emotions" campaign less worried about long-term effects of COVID-19, even less than the Reason campaign (scale 1-5)
      endline_survey$e_long_term_concerns[i] <- sample(1:5, 1, replace = TRUE, prob = c(0.05, 0.2, 0.7, 0.05, 0)) 
# Make those exposed to the "Emotions" campaign less worried about the long-term effects of COVID-19 vaccine, even less than the Reason campaign (scale 1-5)
      endline_survey$e_vaccine_long_term_concerns[i] <- sample(1:5, 1, replace = TRUE, prob = c(0.05, 0.2, 0.7, 0.05, 0))}}}
```

```{r}
# ANALYSIS AND REPORTING
## MERGE DATASETS
endline_survey_subset <- endline_survey %>%
  select(-any_of(intersect(names(endline_survey), names(vaccine_ad_campaigns))[-1]))

vaccine_ad_campaigns_all <- left_join(vaccine_ad_campaigns, endline_survey_subset, by = "id")
```
...
prop_b <- prop.table(table(vaccine_ad_campaigns_all$assignment, vaccine_ad_campaigns_all$b_had_vaccine), margin = 1) * 100
prop_e <- prop.table(table(vaccine_ad_campaigns_all$assignment, vaccine_ad_campaigns_all$e_had_vaccine), margin = 1) * 100
...
education_vaccination <- aggregate(cbind(
  ...
  data = vaccine_ad_campaigns_all, FUN = mean)
...
```{r}
# ARE YOU WORRIED ABOUT COVID?
worried_about_covid <- aggregate(cbind(b_worried_about_covid, e_worried_about_covid) ~ assignment, data = vaccine_ad_campaigns_all, FUN = mean)
...
long_term_covid_concerns <- aggregate(cbind(b_long_term_concerns, e_long_term_concerns) ~ assignment, data = vaccine_ad_campaigns_all, FUN = mean)
...
gender_concerns <- aggregate(cbind(b_long_term_concerns, e_long_term_concerns) ~ gender + assignment, data = vaccine_ad_campaigns_all, FUN = mean)
